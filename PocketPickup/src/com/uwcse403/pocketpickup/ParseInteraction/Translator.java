...
/**
 * Translates between PocketPickup Game objects (for use communicating between
 * activities in the app) and ParseObject Game objects to be stored in the
 * database.
 * 
 */
public final class Translator {
	 ...
	 * 
	 * @param game
	 *            ParseObject Game object
	 * @return Game object corresponding to the Game object stored in the Parse
	 *         database
	 */
	public static Game parseGameToAppGame(ParseObject game) {
		String creatorId = game.getObjectId();
		ParseGeoPoint parseLocation = (ParseGeoPoint) game
				.get(DbColumns.GAME_LOCATION);
		LatLng location = new LatLng(parseLocation.getLatitude(),
				parseLocation.getLongitude());
		...
		Log.v(LOG_TAG,
				"bimap contains it: "
						+ PocketPickupApplication.sportsAndObjs.inverse()
								.containsKey(parseSportObj));
		String gameType = PocketPickupApplication.sportsAndObjs.inverse().get(
				parseSportObj);
		...
		return new Game(creatorId, location, gameStartDate, gameEndDate,
				startTime, endTime, gameType, idealGameSize, gameDetails,
				objectId);
	}
	 ...
	 * 
	 * @param game
	 *            object to be converted into a new ParseObject to be saved to
	 *            the database. This method SHOULD NOT be called if there is
	 *            already a Game object in the database. This is only for
	...
	public static ParseObject appGameToNewParseGame(Game game) {
		// first check to see if this Game object corresponds to a game created
		// by the current
		// user. If so, this means we don't have to fetch the user info from the
		// Parse servers
		...
		Log.v(LOG_TAG, "current ParseUser objectId: "
				+ PocketPickupApplication.userObjectId);
		if (creatorId.equals(PocketPickupApplication.userObjectId)) {
			g.put(DbColumns.GAME_CREATOR, ParseUser.getCurrentUser());
		} else {
			// TODO implement this, if it necessary. Currently it will never be
			// used
			// if it is called, throw an exception
			// throw new IllegalStateException();
		}
		ParseGeoPoint location = new ParseGeoPoint(game.mGameLocation.latitude,
				game.mGameLocation.longitude);
		...
		g.put(DbColumns.GAME_SPORT,
				PocketPickupApplication.sportsAndObjs.get(game.mGameType));
	...
	/**
	 * @param app
	 *            Game object that corresponds to a ParseObject Game object that
	 *            is currently in the database. This method is used for UPDATING
	 *            EXISTING GAMES.
	 * @requires Game object have a valid id field
	 * @return the ParseObject Game object from the database if found, else
	 *         returns null.
	 */
	public static ParseObject appGameToExistingParseGame(Game game) {
		if (game.id == null) {
			throw new IllegalArgumentException(
					"Game object must have non-null id field");
		}
		...
		if (result.size() == 1) {
			...
			Log.e(LOG_TAG, "expected to find exactly one game but found "
					+ result.size());
			return null;
		}
	}
